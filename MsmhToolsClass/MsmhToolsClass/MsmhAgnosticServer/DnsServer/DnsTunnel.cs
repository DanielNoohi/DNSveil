...
{
    public static async Task ProcessAsync(AgnosticResult aResult, AgnosticProgram.Rules rulesProgram, AgnosticProgram.DnsLimit dnsLimitProgram, DnsCache dnsCaches, AgnosticSettings settings, Endless endless, EventHandler<EventArgs>? onRequestReceived)
    {
        try
        {
            ...
            if (dmQ.IsSuccess && dmQ.Header.QuestionsCount > 0 && dmQ.Questions.QuestionRecords.Count > 0)
            {
                ...
                if (!NetworkTool.IsDomainNameValid(addressQ))
                {
                    msgReqEvent += "Request Denied - Not A Valid Domain";
                    // On Linux Servers This Usually Happens, I Don't Write It To Event To Avoid Spam.
                ...
                if (!usedCache)
                {
                    ...
                    {
                        bool resolveDomain = typeQ == DnsEnums.RRType.AAAA || typeQ == DnsEnums.RRType.A;
                        rr = await rulesProgram.GetAsync(aResult.Local_EndPoint.Address.ToStringNoScopeId(), addressQ, 0, settings, endless, resolveDomain, false).ConfigureAwait(false);
                    }
                ...
                if (dmR.IsSuccess)
                {
                    foreach (IResourceRecord answer in dmR.Answers.AnswerRecords)
                    {
                        ...
                        else if (answer is AaaaRecord aaaaRecord) answers.Add(aaaaRecord.IP.ToStringNoScopeId());
